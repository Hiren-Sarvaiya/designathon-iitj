...
import { Map, Lock, Clock, Play, ChevronLeft, CheckCircle, Star } from 'lucide-react';
...
export const MapMaker = () => {
    ...
    const { user } = useGame();
    const currentUnlockedLevel = user.unlockedLevels.directions;
            ...
            <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                <div>
                    ...
                    <h1 className="text-4xl md:text-5xl font-black text-white mb-2 flex items-center gap-4">
                        <span className="bg-green-500/20 p-3 rounded-2xl text-green-400"><Map className="w-10 h-10" /></span>
                        The Map-Maker
                    </h1>
                    <p className="text-slate-400 text-lg max-w-2xl">Follow coordinates, compass directions, and logical paths to find the treasure.</p>
                </div>
...
const PuzzleCard = ({ puzzle, index }) => {
    ...
    return (
        ...
        >
            <Link to={isLocked ? '#' : `/puzzle/directions/${puzzle.id}`}>
                <Card className={cn(
                    "h-full group transition-all duration-300 relative overflow-hidden",
                    isLocked ? "opacity-50 grayscale" : "hover:-translate-y-2 hover:shadow-[0_0_30px_rgba(34,197,94,0.15)] cursor-pointer hover:border-green-500/50"
                )}>
                    {isUnlocked && <div className="absolute inset-0 bg-gradient-to-b from-green-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />}

                    <div className="aspect-video bg-slate-950 rounded-xl mb-4 relative overflow-hidden flex items-center justify-center border border-slate-800 group-hover:border-green-500/30 transition-colors">
                        <Map className="w-12 h-12 text-green-500/40 group-hover:text-green-500 transition-colors" />
                    ...
                    <div className="flex justify-between items-start mb-2">
                        <div>
                            <div className="font-bold text-lg text-slate-200 group-hover:text-green-400 transition-colors">Expedition #{puzzle.id}</div>
                            <div className="text-xs text-slate-500 font-medium">Pathfinding</div>
                        </div>
